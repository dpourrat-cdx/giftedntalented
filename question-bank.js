(function () {
  ...
  function sampleWithoutReplacement(items, count) {
    ...
    for (let index = copy.length - 1; index > 0; index -= 1) {
      const swapIndex = globalThis.secureRandomIndex(index + 1);
  ...
  function buildEventOrderQuestion(section, events, index) {
    ...
    return makeChoiceQuestion(
      ...
      correct,
      [...sequence.filter((value) => value !== correct), "not enough information"].slice(0, 3),
  ...
  function buildLineOrderQuestion(section, names, index) {
    ...
    return makeChoiceQuestion(
      ...
      correct,
      [...order, "not enough information"].filter((value) => value !== correct).slice(0, 3),
